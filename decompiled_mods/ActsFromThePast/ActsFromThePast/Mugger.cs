...
public sealed class Mugger : CustomMonsterModel
{
	...
	public override int MaxInitialHp => AscensionHelper.GetValueIfAscension((AscensionLevel)8, 54, 52);

	private int MugDamage => AscensionHelper.GetValueIfAscension((AscensionLevel)9, 11, 10);

	private int BigSwipeDamage => AscensionHelper.GetValueIfAscension((AscensionLevel)9, 18, 16);

	private int EscapeBlock => AscensionHelper.GetValueIfAscension((AscensionLevel)8, 17, 11);
	...
	private void PlayRandomDeathSfx()
	{
		...
		string text = ((num != 0) ? "mugger_death_2" : "mugger_death_1");
		if (1 == 0)
		{
		}
		string soundName = text;
		AFTPModAudio.Play("mugger", soundName);
	}
	...
	protected override MonsterMoveStateMachine GenerateMoveStateMachine()
	{
		...
		MoveState val4 = new MoveState("BIG_SWIPE", (Func<IReadOnlyList<Creature>, Task>)BigSwipe, (AbstractIntent[])(object)new AbstractIntent[1] { (AbstractIntent)new SingleAttackIntent(BigSwipeDamage) });
	...
	{
		if (_mugCount == 1 && !_hasSpoken && Rng.Chaotic.NextFloat(1f) < 0.6f)
		{
			_hasSpoken = true;
			TalkCmd.Play(MonsterModel.L10NMonsterLookup("ACTSFROMTHEPAST-MUGGER.moves.MUG.banter"), ((MonsterModel)this).Creature, (VfxColor)9, (VfxDuration)4);
		}
		PlayAttackSfx();
	...
	{
		await CreatureCmd.GainBlock(((MonsterModel)this).Creature, (decimal)EscapeBlock, (ValueProp)8, (CardPlay)null, false);
	}

	private async Task Escape(IReadOnlyList<Creature> targets)
	{
		TalkCmd.Play(MonsterModel.L10NMonsterLookup("ACTSFROMTHEPAST-MUGGER.moves.ESCAPE.banter"), ((MonsterModel)this).Creature, (VfxColor)9, (VfxDuration)2);
	...
	private async Task BigSwipe(IReadOnlyList<Creature> targets)
	{
		...
		await FastAttackAnimation.Play(((MonsterModel)this).Creature);
		VfxCmd.PlayOnCreatureCenters((IEnumerable<Creature>)targets, "vfx/vfx_coin_explosion_regular");
		await DamageCmd.Attack((decimal)BigSwipeDamage).FromMonster((MonsterModel)(object)this).WithAttackerFx((string)null, "event:/sfx/enemy/enemy_attacks/gremlin_merc/sneaky_gremlin_attack", (string)null)
			.WithHitFx("vfx/vfx_attack_slash", (string)null, (string)null)
			.Execute((PlayerChoiceContext)null);
		await StealGold();
		_mugCount++;
	...
	private void PlayAttackSfx()
	{
		...
		string text = ((num != 0) ? "mugger_talk_2" : "mugger_talk_1");
		if (1 == 0)
		{
		}
		string soundName = text;
