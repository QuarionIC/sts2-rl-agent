...
public sealed class SpikeSlimeLarge : CustomMonsterModel
{
	...
	public override int MinInitialHp => OverrideHp ?? AscensionHelper.GetValueIfAscension((AscensionLevel)8, 67, 64);

	public override int MaxInitialHp => OverrideHp ?? AscensionHelper.GetValueIfAscension((AscensionLevel)8, 73, 70);

	private int FlameTackleDamage => AscensionHelper.GetValueIfAscension((AscensionLevel)9, 18, 16);

	private int FrailTurns => AscensionHelper.GetValueIfAscension((AscensionLevel)9, 3, 2);
	...
	private async Task FlameTackle(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack((decimal)FlameTackleDamage).FromMonster((MonsterModel)(object)this).WithAttackerFx((string)null, "event:/sfx/enemy/enemy_attacks/twig_slime_s/twig_slime_s_attack", (string)null)
	...
	private async Task Lick(IReadOnlyList<Creature> targets)
	{
		...
		{
			await PowerCmd.Apply<FrailPower>((PlayerChoiceContext)new ThrowingPlayerChoiceContext(), target, (decimal)FrailTurns, ((MonsterModel)this).Creature, (CardModel)null, false);
		}
	}
	...
	private async Task Split(IReadOnlyList<Creature> targets)
	{
		...
		string slot1 = combatState.Encounter.Slots?.FirstOrDefault((string s) => s.StartsWith("spike_med") && !occupiedSlots.Contains(s));
		string slot2 = null;
		if (slot1 != null)
		{
			occupiedSlots.Add(slot1);
			slot2 = combatState.Encounter.Slots?.FirstOrDefault((string s) => s.StartsWith("spike_med") && !occupiedSlots.Contains(s));
		}
		...
		SpikeSlimeMedium slime1 = (SpikeSlimeMedium)(object)((MonsterModel)ModelDb.Monster<SpikeSlimeMedium>()).ToMutable();
		...
		SpikeSlimeMedium slime2 = (SpikeSlimeMedium)(object)((MonsterModel)ModelDb.Monster<SpikeSlimeMedium>()).ToMutable();
