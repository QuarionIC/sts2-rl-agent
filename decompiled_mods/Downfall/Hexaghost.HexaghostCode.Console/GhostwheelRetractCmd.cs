...
public class GhostwheelRetractCmd : AbstractConsoleCmd
{
	...
	public override CmdResult Process(Player? issuingPlayer, string[] args)
	{
		...
		TaskHelper.RunSafely(RetractMultiple((PlayerChoiceContext)new BlockingPlayerChoiceContext(), issuingPlayer, result));
		...
		return new CmdResult(true, $"Retracted {result} step(s). Now at index {currentIndex} ({((object)currentFlame).GetType().Name}).");
	}
